...
jest.mock('@chorus/shared', () => {
  ...
  return {
    ...
    createApiClient: jest.fn(() => ({
      ...
      search: anything(),
      grammar: anything(),
      sparky: { ask: anything(), getJob: anything() },
      moderation: anything(),
      otp: anything(),
      flags: { getMyFlags: anything() },
      api: anything(),
    })),
...
describe('apiService auth boundary', () => {
  it('login forwards {username, password} and returns the unwrapped body', async () => {
    ...
    await expect(apiService.login('alice.en-es@chorus.test', 'ChorusDev2026!')).resolves.toBe(
      body
    );
    expect(mockLogin).toHaveBeenCalledWith({
      username: 'alice.en-es@chorus.test',
      password: 'ChorusDev2026!',
    });
  });
  ...
  it('switchUser resolves {tokens, user} on success', async () => {
    ...
    await expect(apiService.switchUser('bob.es-en@chorus.test', 'ChorusDev2026!')).resolves.toEqual(
      session
    );
  });
