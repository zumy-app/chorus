...
export default function CallScreen({ route, navigation }: any) {
  const { callId, chatId, chatName } = route.params as { callId: string; chatId: string; chatName: string };
  ...
  const renderCaption = ({ item, index }: { item: TranscriptSegment; index: number }) => {
    ...
    return (
      <View style={styles.captionBubble}>
        ...
        {translation && translation !== item.originalText ? (
          <Text style={styles.captionTranslated}>{translation}</Text>
        ) : null}
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          ...
          <Text style={styles.headerSub}>{status === 'active' ? formatDuration(duration) : 'Ended'} · Audio call</Text>
        </View>
      ...
      <View style={styles.transcriptWrap}>
        <View style={styles.transcriptHeader}>
          ...
          <Text style={styles.transcriptHint}>Scrollable · tap Save to add to vocab</Text>
        </View>
        {segments.length === 0 ? (
        ...
        {hasMore ? (
          <TouchableOpacity onPress={() => loadCaptions(segments.length)} style={styles.loadMore}><Text style={styles.loadMoreText}>Load older captions</Text></TouchableOpacity>
        ) : null}
        <View style={styles.inputRow}>
          <TextInput
            ...
            style={styles.input}
            onSubmitEditing={handleSendCaption}
            returnKeyType="send"
          />
          <TouchableOpacity onPress={handleSendCaption} disabled={!captionInput.trim() || sending} style={[styles.sendBtn, (!captionInput.trim() || sending) && styles.sendBtnDisabled]}>
      ...
      <View style={styles.controls}>
        <TouchableOpacity onPress={() => setMuted(v => !v)} style={[styles.ctrlBtn, muted && styles.ctrlBtnActive]}>
          <Text style={[styles.ctrlIcon, muted && styles.ctrlIconActive]}>{muted ? '🔇' : '🎤'}</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setSpeakerOn(v => !v)} style={[styles.ctrlBtn, !speakerOn && styles.ctrlBtnMuted]}>
          <Text style={styles.ctrlIcon}>{speakerOn ? '🔊' : '🔈'}</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={handleEnd} style={styles.endBtn}>
          <Text style={styles.endIcon}>✕</Text><Text style={styles.endText}>End</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  minimizeBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.1)', alignItems: 'center', justifyContent: 'center' },
  minimizeText: { color: '#fff', fontSize: 16 },
  hero: { alignItems: 'center', paddingVertical: 24, backgroundColor: '#132a4a' },
  ...
  controls: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, paddingVertical: 14, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.08)', backgroundColor: '#081428' },
  ctrlBtn: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(255,255,255,0.08)', alignItems: 'center', justifyContent: 'center' },
  ctrlBtnActive: { backgroundColor: '#f59e0b' },
  ctrlBtnMuted: { opacity: 0.5 },
  ctrlIcon: { fontSize: 20 },
  ctrlIconActive: { color: '#fff' },
  endBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#dc2626', paddingHorizontal: 20, height: 48, borderRadius: 24 },
  endIcon: { color: '#fff', fontSize: 16, fontWeight: '700' },
