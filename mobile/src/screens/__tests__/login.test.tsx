...
describe('LoginScreen', () => {
  it('logs in with the typed client, persists tokens+user, navigates to MainTabs', async () => {
    ...
    await fillAndSubmit(ui, 'alice.en-es@chorus.test', 'ChorusDev2026!');

    await waitFor(() => {
      expect(api.login).toHaveBeenCalledWith('alice.en-es@chorus.test', 'ChorusDev2026!');
    });
    await waitFor(() => {
  ...
  it('DevAccountSwitcher Fill populates the form fields', async () => {
    ...
    await waitFor(() => {
      ...
      expect(ui.getByPlaceholderText('Enter your password').props.value).toBe('ChorusDev2026!');
    });
  });
});
