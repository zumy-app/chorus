...
- Learners `alice.en-es@chorus.test` / `bob.es-en@chorus.test` — password `ChorusDev2026!` `:17`
...
| **Depends on** | Auth (`POST /auth/login` → `alice.en-es@chorus.test` / `ChorusDev2026!` `dev_seed.go:17`), `MarketplaceTab` exists. No prereq slice. |
...
Feature: Browse tutors + Find a trial tutor
  ...
  Background:
    ...
    And I am authenticated as alice.en-es@chorus.test (POST /auth/login with ChorusDev2026!)
...
- [ ] **No secrets/outline leaks:** `.env*` untouched, no `WAITLIST_ADMIN_EMAILS` or `SMTP_PASSWORD` printed, `grep -R "sk-" frontend/dist mobile/dist` 0, `grep -R "ChorusDev2026!" frontend/dist` 0.
