...
- Learners `alice.dev@chorus.test` / `bob.dev@chorus.test` — password `ChorusDev2026!` `:17`
...
| **Depends on** | Auth (`POST /auth/login` → `alice.dev@chorus.test` / `ChorusDev2026!` `dev_seed.go:17`), `MarketplaceTab` exists. No prereq slice. |
...
Feature: Browse tutors + Find a trial tutor
  ...
  Background:
    ...
    And I am authenticated as alice.dev@chorus.test (POST /auth/login with ChorusDev2026!)
