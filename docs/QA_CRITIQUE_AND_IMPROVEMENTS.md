...
- **Finding:** Tests depend on two externally-owned Gmail accounts with a shared cleartext password `***REMOVED***` (`users.ts:18,24`). Any password rotation, 2FA, captcha, or Gmail deletion makes the entire 01/03/04/05/06 suite flake. Seed is not deterministic — `dev_seed.go:31` provisions `alice.en-es@chorus.test` / `bob.es-en@chorus.test` / `sofia.tutor@chorus.test` (`DevPassword ChorusDev2026!` `dev_seed.go:17`) but no test uses them. Parallel runs collide on the same Gmail inboxes/chats.
...
`alice.en-es@chorus.test` / `bob.es-en@chorus.test` / `sofia.tutor@chorus.test` password `ChorusDev2026!` (`dev_seed.go:17`), seeded via `global-setup.ts:103` `SeedDevData` + JWT clear.
