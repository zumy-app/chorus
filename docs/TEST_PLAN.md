...
|---|---|---|---|---|
| Legacy Gmail users `uhsarp@gmail.com` / `avcxafefwer@gmail.com` with shared cleartext password | `e2e/fixtures/users.ts:16` | Flake on rotation/captcha, not deterministic, collides parallel | §3.1 switch to `DEV_ACCOUNTS` (`devAccounts.ts:15` alice.dev/bob.dev/sofia.tutor `ChorusDev2026!`) | `grep -R "uhsarp" e2e/` 0 |
