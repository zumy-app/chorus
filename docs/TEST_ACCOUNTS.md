...
> **All use password:** `ChorusDev2026!` (`DevPassword`). Never used in production.

| # | Email | Username | Display name | Native → Target | Role / Data | Password |
|---|---|---|---|---|---|---|
| 1 | `alice.en-es@chorus.test` | `alice.en-es` | `Alice Dev` | `en → es` | **Learner (EN speaker learning ES)** — has 1 trial credit, 2 reviews for Sofia, used in `seedTutorMarketplace` | `ChorusDev2026!` |
| 2 | `bob.es-en@chorus.test` | `bob.es-en` | `Bob Dev` | `es → en` | **Learner (ES speaker learning EN)** — second learner for booking/review flows | `ChorusDev2026!` |
| 3 | `sofia.tutor@chorus.test` | `sofia.tutor` | `Sofia Tutor` | `en → es` | **Approved tutor** — `teacher_applications status=approved`, `rate 2500¢ ($25)`, bio `Hola! I am Sofia...`, `language_certificate` verified `Instituto Cervantes 2018`, 4 future `tutor_availability` slots (1h/day next 4d), ratings 5★+4★ avg 4.5 | `ChorusDev2026!` |
...
# then in app:
# Login as alice.en-es@chorus.test / ChorusDev2026!  → Learn hub shows dueToday>0 + 5 es scenarios
# Login as sofia.tutor@chorus.test / ChorusDev2026! → Profile → Become Teacher shows `approved` + Trial Credits
...
- `.env:VITE_DEFAULT_LOGIN_EMAIL=uhsarp@gmail.com / ***REMOVED***` is unrelated Appwrite legacy, not Chorus DB.
