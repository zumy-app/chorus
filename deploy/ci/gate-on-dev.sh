...
GATES="${GATES:-seed e2e phoenix-eval load-smoke perf release-gate mail-isolation nfr wireframe-parity}"
...
for gate in $GATES; do
  case "$gate" in
    ...
    nfr)            script="verify-nfr.sh" ;;
    wireframe-parity) script="verify-wireframe-parity.sh" ;;
    perf)           script="verify-perf.sh --offline" ;;
