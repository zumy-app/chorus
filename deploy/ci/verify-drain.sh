...
WS_TOKEN="${WS_TOKEN:-}"
if [ -z "$WS_TOKEN" ]; then
  WS_TOKEN=$(curl -s -X POST -H 'Content-Type: application/json' -d '{"username":"uhsarp@gmail.com","password":"***REMOVED***"}' "$BASE_URL/api/v1/auth/login" | sed -n 's/.*"accessToken":"\([^"]*\)".*/\1/p')
fi
