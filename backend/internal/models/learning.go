...
type MonthlyActivityPoint struct {
	Month               string `json:"month"`
	WordsLearned        int    `json:"wordsLearned"`
	SentencesUnderstood int    `json:"sentencesUnderstood"`
}
...
type LearningDashboard struct {
	...
	MonthlyActivity       []MonthlyActivityPoint `json:"monthlyActivity"`
}
...
// LevelSelectionRequest captures an onboarding self-selected starting level
// (beginner | intermediate | advanced). It is the "I know my level" alternative
// to the placement test: the backend seeds the profile's CEFR band, readiness,
// and starting unit from the bucket without running an adaptive assessment.
type LevelSelectionRequest struct {
...
// Placement state constants for user_language_profiles.placement_status.
// self_selected is set via onboarding level self-selection (task 2.3).
const (
	PlacementStatusNotStarted   = "not_started"
	PlacementStatusInProgress   = "in_progress"
	PlacementStatusCompleted    = "completed"
	PlacementStatusSkipped      = "skipped"
	PlacementStatusSelfSelected = "self_selected"
)
...
// ---------------------------------------------------------------------------
// Unified SRS queue (FR-32 — seed + personal + grammar, interleaved with SM-2)
// ---------------------------------------------------------------------------

// Queue origin tags so a client can render seed vs personal vs grammar items
// (seed = curated curriculum lexical items, personal = chat/scenario-mined
// cards, grammar = grammar-point cloze/review items).
const (
...
// SRSQueueItem is a single due/new review item in the unified queue. ID is the
// vocabulary-card id (vocabulary items) or grammar-point id (grammar items).
// Answer carries the expected term/blank so a client can grade a standalone
// queue item before routing it into a practice session.
...
// SRSQueue is the interleaved, SM-2-scheduled review queue for a user/pair.
