...
func userRowWithPlan(plan string) *sqlmock.Rows {
	return sqlmock.NewRows([]string{
		...
		"subscription_plan_id", "subscription_status", "next_billing_date", "last_payment_at", "avatar_url", "phone", "phone_verified", "phone_verified_at", "two_factor_enabled",
	}).AddRow("user-1", "testuser", "test@example.com", "Test User", "", "", "en", pq.Array([]string{"es"}),
		"member", time.Now(), time.Now(), nil, nil, plan, nil, nil, nil, "paypal", nil, nil, nil, nil, nil, nil, false, nil, false)
}
