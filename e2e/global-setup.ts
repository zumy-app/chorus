  ...
  if (process.env.E2E_PREWARM !== 'false') {
    ...
    try {
      ...
      const loginRes = await fetch(`${apiUrl}/auth/login`, {
        ...
        body: JSON.stringify({ email: 'alice.dev@chorus.test', password: 'ChorusDev2026!' }),
      })
      if (loginRes.ok) {
        ...
        if (token) {
          ...
          if (!chatId) {
            ...
            try {
              const bobLogin = await fetch(`${apiUrl}/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: 'bob.dev@chorus.test', password: 'ChorusDev2026!' }) })
