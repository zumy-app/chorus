...
export default function RealTalkHub() {
  ...
  const useInChat = useCallback(async (prompt: RealTalkPrompt) => {
    ...
    // Draft the sendable target-language phrase, never the instruction.
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 overflow-y-auto px-margin-mobile pt-4 pb-32 max-w-md w-full mx-auto flex flex-col gap-4">
        <header>
          <h1 className="font-headline-lg text-headline-lg text-on-surface">{t('realTalk.title')}</h1>
          <p className="font-body-sm text-body-sm text-on-surface-variant">{t('realTalk.subtitle')}</p>
        </header>
        ...
        {currentUnit && (
          <div className="bg-surface-container-high rounded-xl p-4 border-l-4 border-primary relative overflow-hidden shadow-sm ring-1 ring-primary/10">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-on-surface-variant font-label-sm text-label-sm uppercase tracking-wider">{t('realTalk.currentGoal')}</span>
            </div>
        ...
        <div className="flex overflow-x-auto gap-2 pb-1 -mx-margin-mobile px-margin-mobile">
          {CATEGORIES.map(c => (
            ...
            >
              {c === 'All' ? t('realTalk.tabsAll') : c}
            </button>
          ))}
        </div>

        {loading && <p className="font-body-md text-body-md text-on-surface-variant mt-2">{t('realTalk.loading')}</p>}
        {!loading && filtered.length === 0 && (
          <div className="mt-2 bg-surface-container-lowest rounded-xl p-6 text-center">
            <p className="font-body-md text-body-md text-on-surface-variant">{t('realTalk.empty')}</p>
            <button onClick={() => setTab('All')} className="mt-3 text-primary font-label-md text-label-md">{t('realTalk.showAll')}</button>
          </div>
        )}
          ...
          {filtered.map(prompt => (
            <div key={prompt.id} className="bg-surface-container-lowest rounded-xl p-4 shadow-sm border border-surface-variant flex flex-col gap-3 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between gap-2">
                ...
                {usedId === prompt.id && <span className="font-label-sm text-label-sm text-tertiary flex items-center gap-1"><span className="material-symbols-outlined text-[14px]">check</span> {t('realTalk.used')}</span>}
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-surface-variant/50">
                <span className="font-body-sm text-body-sm text-outline flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">lightbulb</span> {t('realTalk.tryTarget')}</span>
                <button onClick={() => useInChat(prompt)} className="bg-primary text-on-primary font-label-md text-label-md px-6 py-2 rounded-full hover:bg-primary/90 transition active:scale-95 flex items-center gap-2">
                  {t('realTalk.useInChat')} <span className="material-symbols-outlined text-[18px]">send</span>
                </button>
              </div>
            </div>
          ))}
